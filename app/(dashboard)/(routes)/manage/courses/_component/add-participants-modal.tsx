...
export function AddParticipantsModal({ courseId, isOpen, onClose, onSuccess }: AddParticipantsModalProps) {
  ...
  const handleRemoveParticipant = async (enrollmentId: string) => {
    ...
    try {
      const response = await fetch(`/api/courses/${courseId}/participants/${enrollmentId}`, {
        method: 'DELETE',
      })
      ...
      <DialogContent className="max-w-2xl max-h-[80vh] overflow-hidden flex flex-col">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            Aggiungi Partecipanti al Corso
          </DialogTitle>
          <DialogDescription>
            Seleziona i membri dell'organizzazione da iscrivere al corso.
          </DialogDescription>
        </DialogHeader>
